...
public class TimestampNormalizer {
	...
	public static void main(String[] args) {
		...
		if (args.length < 1) {
			LOG.critical("Missing arguments. Please give argument imputData");
			return;
		}
		...
		File normalizedFile = new File(pathWithoutExtension + "_normalized" + extension);
		...
		try (BufferedReader dataReader = new BufferedReader(new FileReader(dataFile));
			...
			while (line != null) {
				...
				List<String> timestampStrings = getTimeStampStrings(splitLine[startTimeIndex], splitLine[carePathIndex]);
	...
	private static List<String> getTimeStampStrings(String startTimeString, String pathString) {
		...
		for(String pathElement : splitPath) {
			if(pathElement.charAt(0) == '(') {
				...
				lines.add(splitElement[0] + ";" + startTime.toString().replace("T", " ") + ":00;" + endTime.toString().replace("T", " ") + ":00");
				startTime = endTime;
			}
		}
		return lines;
